...
const Document_Tracker = 'DocumentTracker.csv';  
...
// Read CSV file
async function readCSV(CSV_FILE) {
    try {
...
async function handleRequest(req, res) {
    ...
    if (pathname === '/api/DocumentTracker' && req.method === 'GET') {
        try {
            const data = await readCSV(Document_Tracker);
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ success: true, data: data }));
        } catch (error) {
            res.writeHead(500, { 'Content-Type': 'application/json' });
